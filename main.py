...
if __name__ == "__main__":
    ...
    if "--status" in args:
        ...
        print(f"     By Niche   : {status['by_niche']}")
        print(f"     By Priority: {status['by_priority']}")
        print()
        print(f"  Avg Confidence     : {status['avg_confidence']}")
        print(f"  Avg Validation     : {status['avg_validation_score']}")
        print(f"  Validated          : {status['validated']}")
        print()
        print(f"  Module Stats:")
        print(f"     Classifier   : {status['classifier']['total_classified']} classified")
        print(f"     Website      : {status['website_mapper']['total_mapped']} mapped")
        print(f"     Account      : {status['account_mapper']['total_mapped']} mapped")
        print(f"     Board        : {status['board_mapper']['total_mapped']} mapped")
        print(f"     Pin Strategy : {status['pin_strategy']['total_analyses']} analyses")
        print(f"     Affiliate    : {status['affiliate_mapper']['total_mapped']} mapped")
        print(f"     SEO          : {status['seo_mapper']['total_seo_profiles']} profiles")
        print(f"     Image        : {status['image_mapper']['total_mapped']} mapped")
        print(f"     Schedule     : {status['scheduling']['total_scheduled']} scheduled")
        print(f"     Validator    : {status['validator']['total_validated']} validated")
        print(f"     Relationships: {status['relationships']['total_relationships']} built")
        print(f"     Recommender  : {status['recommendations']['total_recommendations']} made")
        print("=" * 60)
        print(json.dumps(status, indent=2, default=str))
