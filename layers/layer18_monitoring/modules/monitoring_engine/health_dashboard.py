...
class HealthDashboard:
    ...
    def update_component(self, name: str, score: int, status: str = "healthy",
        ...
        if not name.strip() or status not in {"healthy", "degraded", "unhealthy"}:
            raise ValueError("invalid component name or status")
        with self._lock:
            self._components[name] = {"name": name, "score": max(0, min(100, int(score))),
                                     "status": status, "last_check": time.time(),
    ...
    def get_overall_health(self) -> Dict[str, Any]:
        ...
        if not components:
            return {"score": 0, "status": "unknown", "component_count": 0, "components": {}}
        ...
        result = {"score": score, "status": status, "component_count": len(components),
                  "components": {k: {"score": v["score"], "status": v["status"]} for k, v in components.items()}}
        with self._lock:
            self._history.append({"timestamp": time.time(), "score": score, "status": status,
    ...
    def get_unhealthy_components(self) -> List[Dict[str, Any]]:
        with self._lock:
            return [dict(c) for c in self._components.values()
    ...
    def get_score_trend(self, window: int = 20) -> Dict[str, Any]:
        ...
        diff = second - first
        return {"trend": "improving" if diff > 5 else "declining" if diff < -5 else "stable",
                "avg_first": round(first, 1), "avg_second": round(second, 1), "scores": scores}

    def stats(self) -> Dict[str, Any]:
        with self._lock:
            components = list(self._components.values())
            return {"total_components": len(components),
                    "healthy": sum(c["status"] == "healthy" for c in components),
                    "degraded": sum(c["status"] == "degraded" for c in components),
                    "unhealthy": sum(c["status"] == "unhealthy" for c in components),
