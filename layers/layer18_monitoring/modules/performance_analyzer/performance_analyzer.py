...
class PerformanceAnalyzer:
    ...
    def set_threshold(
        self, metric_name: str, min_val: float, max_val: float
    ) -> None:
    ...
    def get_history(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            return [item.to_dict() for item in self._snapshots[-limit:]]
    ...
    def check_violations(
        ...
        for name, value in metrics.items():
            ...
            if threshold and (
                value < threshold["min"] or value > threshold["max"]
            ):
                violations.append(
                    {
                        "metric": name,
    ...
    def summary(self) -> Dict[str, Any]:
        with self._lock:
            if not self._snapshots:
                return {"snapshots": 0}
            scores = [snapshot.score for snapshot in self._snapshots]
            return {
