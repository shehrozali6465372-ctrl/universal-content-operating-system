...
class GlobalMemory:
    """System-wide bounded memory center with a consistent secondary index."""
    ...
    def get_most_accessed(self, count: int = 10) -> List[MemoryEntry]:
        with self._lock:
            return sorted(
                self._entries, key=lambda entry: entry.access_count, reverse=True
            )[:count]

    def get_most_important(self, count: int = 10) -> List[MemoryEntry]:
        with self._lock:
            return sorted(
                self._entries, key=lambda entry: entry.importance, reverse=True
            )[:count]
    ...
    def clear(self, memory_type: str = "") -> int:
        with self._lock:
            if memory_type:
                to_remove = [
                    entry for entry in self._entries if entry.memory_type == memory_type
                ]
