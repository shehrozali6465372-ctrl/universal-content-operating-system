...
class MemoryVersioner:
    ...
    def get_version(self, entry_id: str, version_number: int) -> Optional[MemoryVersion]:
        with self._lock:
            for version in self._versions.get(entry_id, []):
    ...
    def get_history(self, entry_id: str) -> List[MemoryVersion]:
        with self._lock:
            return copy.deepcopy(self._versions.get(entry_id, []))

    def rollback(self, entry_id: str, to_version: int) -> Optional[MemoryVersion]:
        if to_version < 1:
            raise ValueError("to_version must be >= 1")
        target = self.get_version(entry_id, to_version)
        if target is None:
            return None
        return self.create_version(entry_id, target.data, change_summary=f"Rollback to version {to_version}")
