"""Thread-safe bounded intelligence store with consistent indexes."""
...
class IntelligenceEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"entry_id": self.entry_id,"category": self.category,"confidence": round(self.confidence,3),
                "score": round(self.score,3),"tags": list(self.tags),"source": self.source,
                "version": self.version,"access_count": self.access_count}
...
class IntelligenceStore:
    ...
    def __init__(self, max_size: int = 1000) -> None:
        if max_size < 1: raise ValueError("max_size must be >= 1")
    ...
    def store(self, category: str, data: Dict[str, Any], confidence: float = 0.5,
              score: float = 0.0, tags: Optional[List[str]] = None, source: str = "") -> IntelligenceEntry:
        if not 0.0 <= confidence <= 1.0: raise ValueError("confidence must be between 0 and 1")
        with self._lock:
            entry = IntelligenceEntry(category, data)
            entry.confidence, entry.score, entry.tags, entry.source = confidence, score, list(tags or []), source
            ...
            for tag in entry.tags: self._tag_index.setdefault(tag, []).append(entry.entry_id)
            return entry

    def get(self, entry_id: str) -> Optional[IntelligenceEntry]:
        with self._lock:
            entry = self._entries.get(entry_id)
            if entry: entry.access_count += 1
            return entry

    def get_all(self) -> List[IntelligenceEntry]:
        with self._lock: return list(self._entries.values())
    ...
    def update(self, entry_id: str, data: Optional[Dict[str, Any]] = None,
               confidence: Optional[float] = None, score: Optional[float] = None,
               tags: Optional[List[str]] = None, source: Optional[str] = None) -> Optional[IntelligenceEntry]:
        with self._lock:
            entry = self._entries.get(entry_id)
            ...
            if tags is not None:
                for ids in self._tag_index.values(): ids[:] = [i for i in ids if i != entry_id]
            ...
            entry.version += 1; entry.updated_at = time.time()
            return entry

    def delete(self, entry_id: str) -> bool:
        with self._lock:
            if entry_id not in self._entries: return False
            self._remove_entry(entry_id); return True
    ...
    def _remove_entry(self, entry_id: str) -> None:
        ...
        for tag in entry.tags:
            ...
            if entry_id in tids: tids.remove(entry_id)
            if not tids: self._tag_index.pop(tag, None)
    ...
    def stats(self) -> Dict[str, Any]:
        with self._lock:
            ...
            return {"total":len(entries),"avg_confidence":round(sum(e.confidence for e in entries)/len(entries),3),
                    "categories":len(self._category_index),"tags":len(self._tag_index)}
