"""cohere_provider.py — Cohere provider implementation."""
...
class CohereProvider(BaseProvider):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._supported_models = ["command-r-plus", "command-r", "command",
                                   "c4ai-command-r-v01", "embed-english-v3.0"]
    ...
    def generate(self, request: ProviderRequest) -> ProviderResponse:
        start = time.time()
        req_id = f"cohere_{next(_REQUEST_ID)}"
        content = f"[Cohere/{request.model}] Generated for: {request.prompt[:100]}..."
        tokens = max(10, len(request.prompt.split()) * 2)
        resp = ProviderResponse(content, request.model or "command-r-plus", "cohere")
    ...
    def chat(self, messages: List[Dict[str, str]], model: str = "") -> ProviderResponse:
        ...
        req = ProviderRequest(prompt, model or "command-r-plus", "cohere")
