...
class OpenAIProvider(BaseProvider):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        super().__init__("openai", config); cfg=config or {}
        ...
        try: self._timeout=float(cfg.get("timeout",60.0))
        except (TypeError,ValueError) as exc: raise ValueError("OpenAI timeout must be numeric") from exc
        if self._timeout<=0 or self._timeout>300: raise ValueError("OpenAI timeout must be greater than 0 and at most 300 seconds")
        self._max_retries=int(cfg.get("max_retries",3))
        if self._max_retries<0: raise ValueError("OpenAI max_retries must be non-negative")
    ...
    def initialize(self)->bool:
        if not self._api_key and self._is_production():self._is_initialized=False;self._health_status="unhealthy";return False
        self._is_initialized=True;self._health_status="healthy" if self._api_key else "simulated";return True
    def _call(self,messages:List[Dict[str,str]],model:str,request:ProviderRequest)->ProviderResponse:
        if not self._api_key:
            if self._is_production():raise RuntimeError("OpenAI provider has no API key; refusing simulated production output")
            tokens=max(10,len(request.prompt.split())*2);response=ProviderResponse(f"[OpenAI/{model}] Simulated for: {request.prompt[:100]}...",model,"openai");response.request_id=f"openai_{uuid.uuid4().hex}";response.usage={"prompt_tokens":tokens,"completion_tokens":tokens*2,"total_tokens":tokens*3};self._metrics["requests"]+=1;self._metrics["total_tokens"]+=response.usage["total_tokens"];return response
        ...
        try:data=json.dumps(payload).encode("utf-8")
        except (TypeError,ValueError) as exc:self._metrics["errors"]+=1;raise RuntimeError("OpenAI request payload is not serializable") from exc
        start=time.time();last_error=None;body={}
        for attempt in range(self._max_retries+1):
            ...
            try:
                with urllib.request.urlopen(req,timeout=self._timeout) as resp:body=json.loads(resp.read().decode("utf-8"))
                last_error=None;break
            except urllib.error.HTTPError as exc:
                last_error=exc
                if exc.code not in (429,500,502,503,504):break
            except (urllib.error.URLError,TimeoutError,OSError,json.JSONDecodeError) as exc:last_error=exc
            if attempt<self._max_retries:time.sleep(min(2**attempt,8))
        if last_error is not None:self._metrics["errors"]+=1;self._health_status="degraded";raise RuntimeError(f"OpenAI request failed: {type(last_error).__name__}") from last_error
        choices=body.get("choices") or []
        if not choices:self._metrics["errors"]+=1;raise RuntimeError("OpenAI response contained no choices")
    ...
    def chat(self,messages:List[Dict[str,str]],model:str="")->ProviderResponse:
        request=ProviderRequest("",model or "gpt-4o-mini","openai");request.messages=list(messages);return self.generate(request)
    def is_available(self)->bool:return self._is_initialized and (bool(self._api_key) or not self._is_production())
