...
class OpenAIProvider(BaseProvider):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self._timeout <= 0 or self._timeout > 300:
            raise ValueError(
                "OpenAI timeout must be greater than 0 and at most 300 seconds"
            )
    ...
    def _call(
        ...
        if not self._api_key:
            if self._is_production():
                raise RuntimeError(
                    "OpenAI provider has no API key; refusing simulated "
                    "production output"
                )
            tokens = max(10, len(request.prompt.split()) * 2)
            response = ProviderResponse(
                f"[OpenAI/{model}] Simulated for: "
        ...
        try:
            ...
            raise RuntimeError(
                "OpenAI request payload is not serializable"
            ) from exc
