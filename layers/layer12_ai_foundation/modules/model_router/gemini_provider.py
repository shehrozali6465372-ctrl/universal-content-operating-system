...
class GeminiProvider:
    ...
    SUPPORTED_MODELS = [
        "gemini-3.8-flash", "gemini-3.7-flash", "gemini-3.6-flash", "gemini-3.5-flash", "gemini-3.5-flash-lite", "gemini-3.1-flash-lite", "gemini-3-flash-preview",
    ...
    def generate(self, prompt: str, model: str = "",
        ...
        last_error = "All API keys failed or unavailable"
        ...
        for _ in range(max_attempts):
            ...
            if result is not None:
                ...
                self._remember({**result, "time": time.time()})
                return result

            self._error_count += 1
            last_error = self._last_error or "api_call_failed"
            if self._key_manager:
                self._key_manager.report_error(
                    key_id_used,
                    last_error,
                    is_rate_limit=(self._last_http_status == 429),
                )
        ...
        self._remember({**error_result, "time": time.time()})
        return error_result
    ...
    def chat(self, messages: List[Dict[str, str]], model: str = "",
        ...
        for _ in range(max_attempts):
            ...
            self._error_count += 1
            last_error = self._last_error or "chat_api_failed"
            if self._key_manager:
                self._key_manager.report_error(
                    key_id_used,
                    last_error,
    ...
    def _max_key_attempts(self) -> int:
        """Bound retries to the configured retry budget and available keys."""
        if not self._key_manager:
            return 0
        try:
            available = len(self._key_manager.get_healthy_keys())
        except Exception:
            available = 0
    ...
    def _real_api_call(self, prompt: str, model: str, api_key: str,
        ...
        url = f"{self._config.api_base}/models/{model}:generateContent"
        self._last_http_status = 0
