...
class AIOrchestrator:
    ...
    def link_module(self, name: str, module: Any) -> None:
        ...
        if not callable(self._method(module, "evaluate")) and not callable(
            self._method(module, "check")
        ):
            raise TypeError(
                f"module '{name}' must expose callable evaluate() or check()"
            )
    ...
    def process(
        ...
        if self._is_production() and not self._is_running:
            return {
                "success": False,
                "error": "AI orchestrator is not running",
        ...
        try:
            if component is None:
                if self._is_production():
                    raise RuntimeError(
                        f"No linked production component for route '{target}'"
                    )
                result = {
                    "component": target,
                    "task": task,
            ...
            elif callable(self._method(component, "evaluate")):
                result = self._method(component, "evaluate")(
                    str(input_data)
                )
            elif callable(self._method(component, "check")):
                result = self._method(component, "check")(str(input_data))
            else:
                raise TypeError(
                    f"Linked component '{target}' has no supported "
                    "execution method"
                )
            if not isinstance(result, dict):
                raise TypeError(
                    f"Linked component '{target}' returned non-dict result"
                )
        except Exception as exc:
            logger.exception(
                "AI orchestration task failed: task=%s", task
            )
        ...
        self.events.publish(
            "task_completed", {"task": task, "success": success}
        )
        return result
    ...
    def generate_report(self) -> Dict[str, Any]:
        return self.report_gen.generate(
            self.metrics.to_dict(), self.health.overall_health()
        )
