...
class ControlPlane:
    ...
    def execute(self,topic:str,platform:Optional[str]=None,account_id:Optional[str]=None,tone:str="professional",style:str="educational",include_image:bool=True)->Dict[str,Any]:
        ...
        if account_id:
            ...
            product=AffiliateEvidenceProvider().select(topic,account.niche,account.platform)
            ...
            product=AffiliateEvidenceProvider().select(decision.topic,decision.niche,decision.platform)
            decision=Decision(**{**decision.__dict__,"product":product,"affiliate":product})
        else:
        ...
        if decision.product:
            style_suffix=f"; verified affiliate candidate: {decision.product['title']} | {decision.product['url']}. Mention/link it only when relevant and permitted by platform/account policy."
            request.style=style+style_suffix
        request.metadata.update({"account_id":decision.account_id,"niche":decision.niche,"content_type":decision.content_type,"policy_version":decision.policy_version,"product":decision.product,"affiliate":decision.affiliate,"control_plane":"account_decision_engine"})
