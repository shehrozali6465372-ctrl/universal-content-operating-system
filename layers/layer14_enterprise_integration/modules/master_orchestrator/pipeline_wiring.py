...
Single execution path: research -> intelligence -> writing -> AI -> image ->
quality -> real platform publishing -> platform analytics -> learning -> persistence.
"""
...
class ContentRequest:
    ...
    def __init__(self, topic: str, platform: str = "facebook", tone: str = "professional", style: str = "educational", include_image: bool = True, max_length: int = 1000) -> None:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"topic": self.topic, "platform": self.platform, "tone": self.tone, "style": self.style, "include_image": self.include_image, "max_length": self.max_length, "metadata": self.metadata}
...
class PipelineStepResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"layer": self.layer, "status": self.status, "duration_ms": round(self.duration_ms, 1), "error": self.error, "data_keys": list(self.data.keys())}


class ContentResponse:
    __slots__ = ("request", "steps", "text", "image_prompt", "image_url", "quality_score", "quality_report", "publish_package", "publish_result", "analytics", "learning_entries", "total_duration_ms", "stats")
...
class PipelineLogger:
    ...
    def log(self, layer: str, event: str, data: Optional[Dict[str, Any]] = None) -> None:
        ...
        icon = "❌" if "error" in event or "failed" in event else "✅" if "success" in event else "🔄"
...
class PipelineWiring:
    ...
    def _writing(self, req: ContentRequest, ctx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = PlannerManager().create_plan(topic=req.topic, platform=req.platform, user_goal="educate", audience_hint="general", tone_override=req.tone)
    ...
    def _ai(self, req: ContentRequest, ctx: Dict[str, Any], response: ContentResponse) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            "and end with a useful call to action. Do not invent statistics or sources. Return only the final post."
        )
        result = self._gemini.generate(prompt, system_prompt=f"You are an expert {req.platform} content writer. Prioritize factual accuracy, clarity, platform conventions, and non-spammy engagement.")
    ...
    def _publisher(self, req: ContentRequest):
        ...
        credentials = {"page_id": os.environ.get("FACEBOOK_PAGE_ID", ""), "access_token": os.environ.get("FACEBOOK_ACCESS_TOKEN", "")}
    ...
    def _publish(self, req: ContentRequest, response: ContentResponse, ctx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        data = {"success": bool(result.success), "platform": req.platform, "post_id": result.post_id, "url": result.url, "error": result.error_message, "duration_ms": result.duration_ms}
    ...
    def _learning(self, req: ContentRequest, response: ContentResponse, ctx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lesson = Lesson(lesson_type="published_content", title=f"Published '{req.topic}' to {req.platform}")
        lesson.description = f"quality={response.quality_score}; published={bool(response.publish_result and response.publish_result.get('success'))}; analytics={response.analytics or {}}"
    ...
    def execute(self, request: ContentRequest) -> ContentResponse:
        ...
        for layer, fn in required_steps:
        ...
        response.total_duration_ms = (time.time() - started) * 1000
        response.stats = {"execution_time_ms": round(response.total_duration_ms, 1), "published": True, "post_id": ctx.get("post_id", "")}
        self._persist(response)
        return response
    ...
    def status(self) -> Dict[str, Any]:
        ...
        return {"pipeline": "canonical", "ai_engine": "gemini" if self._gemini else "unavailable", "api_keys_configured": stats.get("total_keys", 0), "healthy_keys": stats.get("healthy", 0)}
