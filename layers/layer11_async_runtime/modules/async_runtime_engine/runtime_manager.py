...
class RuntimeManager:
    """Own the runtime engine and its lifecycle state machine."""
    ...
    def __init__(self, config: Optional[RuntimeConfig] = None) -> None:
        ...
        self.memory = RuntimeMemory(max_checkpoints=self.config.max_tasks)
        ...
        self.health.register_check("runtime", self._runtime_health)
    ...
    def start(self) -> bool:
        with self._lock:
            ...
            try:
                ...
                self.state.transition(RuntimeState.ERROR)
                self.events.publish("runtime_start_failed", "manager")
                return False
    ...
    def stop(self) -> bool:
        with self._lock:
            ...
            try:
                ...
                self.state.transition(RuntimeState.ERROR)
                self.events.publish("runtime_stop_failed", "manager")
                return False
    ...
    def restart(self) -> bool:
        with self._lock:
            ...
            if current not in {
                ...
                return False
            if current in {RuntimeState.RUNNING, RuntimeState.PAUSED}:
                if not self.stop():
                    return False
            elif current == RuntimeState.ERROR:
