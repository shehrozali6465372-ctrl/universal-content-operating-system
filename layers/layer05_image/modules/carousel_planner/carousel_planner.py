...
SUPPORTED_CAROUSEL_PLATFORMS = {"facebook", "instagram", "linkedin", "pinterest", "threads"}

class CarouselSlide:
    """A single carousel slide."""
    __slots__ = ("slide_number", "title", "content", "image_prompt", "layout", "is_cover", "is_cta")
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"slide_number": self.slide_number, "title": self.title, "content": self.content, "is_cover": self.is_cover, "is_cta": self.is_cta}
...
class CarouselPlan:
    ...
    def __init__(self, topic: str = "", platform: str = "instagram") -> None:
        ...
        self.total_slides = 0
    def to_dict(self) -> Dict[str, Any]:
        return {"plan_id": self.plan_id, "topic": self.topic, "platform": self.platform, "total_slides": self.total_slides, "slides": [s.to_dict() for s in self.slides]}

class CarouselPlanner:
    """Plans carousel content for social platforms."""
    def __init__(self) -> None:
        self._plan_count = 0
    def plan(self, topic: str, platform: str = "instagram", key_points: Optional[List[str]] = None, slide_count: int = 5) -> CarouselPlan:
        ...
        points = key_points or [f"Point {i+1} about {topic.strip()}" for i in range(slide_count - 2)]
