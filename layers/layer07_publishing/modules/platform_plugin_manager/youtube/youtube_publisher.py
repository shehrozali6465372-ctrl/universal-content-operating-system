...
class YouTubePublisher(BasePublisher):
    ...
    def get_capabilities(self):
        c=PlatformCapabilities(); c.supports_video=True; c.supports_images=False; c.supports_analytics=True; c.supports_edit=True; c.supports_delete=True; c.max_length=5000; c.features=["video_upload","video_metadata","statistics"]; return c
    def authenticate(self, credentials):
        self.token=credentials.get("access_token") or os.getenv("YOUTUBE_ACCESS_TOKEN","")
        if not self.token: return False
        try:
            data=self._get("/channels",{"part":"id","mine":"true"}); self.authenticated=bool(data and data.get("items"))
        except Exception: self.authenticated=False
        return self.authenticated
    def validate(self, content, content_type="video"):
    ...
    def publish(self, content, media_paths=None, content_type="video", **kwargs):
        ...
        if not media_paths: r.error_message="YouTube publishing requires a video file"; return r
        path=media_paths[0]
        if not os.path.isfile(path): r.error_message="YouTube publisher requires a local video file"; return r
        try:
            title=kwargs.get("title") or content.splitlines()[0][:100] or "UCOS video"
            body={"snippet":{"title":title,"description":content},"status":{"privacyStatus":kwargs.get("privacy_status","private")}}
            result=self._upload(path,body)
            vid=result.get("id") if result else None
            if vid: r.success=True; r.post_id=vid; r.url=f"https://www.youtube.com/watch?v={vid}"; r.metadata={"privacy_status":body["status"]["privacyStatus"]}
    ...
    def delete(self, post_id):
        try: self._request("DELETE",f"/videos",{"id":post_id},None); return True
    ...
    def schedule(self, content, scheduled_time, media_paths=None, **kwargs):
        r=PublishResult(platform="youtube"); r.error_message="Use privacyStatus=private and publishAt through the YouTube API when scheduling is explicitly configured"; return r
    def _upload(self,path,body):
        ...
        req=urllib.request.Request(self.UPLOAD+"?uploadType=resumable&part=snippet,status",data=meta,method="POST",headers={"Authorization":f"Bearer {self.token}","Content-Type":"application/json; charset=UTF-8","X-Upload-Content-Type":mime,"X-Upload-Content-Length":str(size)})
