...
class FacebookPublisher(BasePublisher):
    ...
    def authenticate(self, credentials: Dict[str, str]) -> bool:
        ...
        token = credentials.get("access_token") or os.environ.get("FACEBOOK_ACCESS_TOKEN", "")
        if not self._page_id or not token:
            self._authenticated = False
            return False

        # First assume the configured token is already a page token.  This is
        # read-only validation and has no publishing side effects.
        ...
        # If it is a user token, resolve the requested page token using the
        # read-only /me/accounts endpoint.
        try:
    ...
    def publish(self, content: str, media_paths: Optional[List[str]] = None, content_type: str = "post", **kwargs: Any) -> PublishResult:
        ...
        try:
            ...
            if payload.get("id"):
                result.success = True
                result.post_id = str(payload["id"])
                result.url = f"https://facebook.com/{result.post_id}"
            ...
            result.error_message = self._http_error(exc)
            self._error_count += 1
            if exc.code == 429:
    ...
    def _publish_with_media(self, content: str, media_paths: List[str], **kwargs: Any) -> Dict[str, Any]:
        ...
        return self._post(f"/{self._page_id}/feed", {
            "message": content,
    ...
    def upload_image(self, image_path: str, caption: str = "") -> PublishResult:
        ...
        try:
            if image_path.startswith(("http://", "https://")):
                data = {"url": image_path, "caption": caption, "published": "false"}
                payload = self._post(f"/{self._page_id}/photos", data)
            else:
    ...
    def delete(self, post_id: str) -> bool:
        try:
            return bool(self._delete(f"/{post_id}").get("success"))
        except Exception:
            return False

    def get_post(self, post_id: str) -> Optional[Dict[str, Any]]:
        try:
            return self._api_get(f"/{post_id}", {"fields": "id,message,created_time,shares,reactions.summary(true),comments.summary(true)"})
        except Exception:
            return None
    ...
    def schedule(self, content: str, scheduled_time: float, media_paths: Optional[List[str]] = None, **kwargs: Any) -> PublishResult:
        result = PublishResult(platform="facebook")
        try:
            payload = self._post(f"/{self._page_id}/feed", {
                "message": content,
                "published": "false",
            ...
            if payload.get("id"):
                result.success = True
                result.post_id = str(payload["id"])
                result.metadata = {"scheduled_time": int(scheduled_time)}
            else:
                result.error_message = str(payload.get("error") or payload)
        except Exception as exc:
            result.error_message = str(exc)
    ...
    def get_page_info(self) -> Dict[str, Any]:
        try:
            return self._api_get(f"/{self._page_id}", {"fields": "id,name,fan_count,followers_count,category"})
        except Exception:
            return {}
    ...
    def _api_get(self, path: str, params: Optional[Dict[str, Any]] = None, token: Optional[str] = None) -> Dict[str, Any]:
        ...
        query["access_token"] = token or self._access_token
    ...
    def _post(self, path: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        request = urllib.request.Request(f"{self.API_BASE}{path}", data=encoded, method="POST")
        with urllib.request.urlopen(request, timeout=60) as response:
            return json.loads(response.read().decode("utf-8"))
    ...
    def _http_error(exc: urllib.error.HTTPError) -> str:
        try:
            ...
            return f"Facebook HTTP {exc.code}: {exc.reason}"
