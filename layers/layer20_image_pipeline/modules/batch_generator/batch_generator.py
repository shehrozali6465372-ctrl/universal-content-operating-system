...
class BatchJob:
    ...
    def __init__(self, prompts: List[Dict[str, Any]]) -> None:
        ...
        self.prompts = [dict(prompt) for prompt in prompts]
...
class BatchGenerator:
    ...
    def execute_batch(self, batch_id: str) -> Dict[str, Any]:
        with self._lock:
            ...
            if self._generator_fn is None:
                ...
                job.errors.append(
                    {"index": None, "error": "generator_not_configured"}
                )
        ...
        with self._lock:
            job.status = (
                BatchStatus.COMPLETED if not job.errors else BatchStatus.FAILED
            )
            job.finished_at = time.time()
            return job.to_dict()
    ...
    def list_batches(self) -> List[Dict[str, Any]]:
        with self._lock:
            return [batch.to_dict() for batch in self._batches.values()]
