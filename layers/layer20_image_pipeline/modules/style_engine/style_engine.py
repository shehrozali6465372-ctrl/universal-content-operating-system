...
class StyleEngine:
    ...
    def add_preset(self, preset: StylePreset) -> None:
        if not isinstance(preset, StylePreset):
            raise TypeError("preset must be a StylePreset")
    ...
    def apply_style(
        self, content: Dict[str, Any], style_name: str
    ) -> Dict[str, Any]:
    ...
    def list_presets(self) -> List[Dict[str, Any]]:
        return [preset.to_dict() for preset in self._presets.values()]
    ...
    def suggest_style(self, platform: str, content_type: str = "post") -> str:
        ...
        return suggestions.get(
            (platform.strip().lower(), content_type.strip().lower()),
            "photorealistic",
        )
