...
class CompositionPlan:
    ...
    def __init__(
        ...
        self.plan_id = f"comp_{uuid.uuid4().hex}"
        self.layout = layout
    ...
    def add_element(
        ...
        self.elements.append(
            {"type": element_type, "position": position, "size": size}
        )
...
class CompositionEngine:
    ...
    def create_plan(
        ...
        plan.metadata["layout_config"] = dict(self._layouts[layout])
        return plan
    ...
    def add_layout(self, name: str, config: Dict[str, Any]) -> None:
        ...
        if not isinstance(config, dict):
            raise TypeError("layout config must be a dictionary")
        self._layouts[name] = dict(config)

    def add_rule(self, rule: CompositionRule) -> None:
        if not isinstance(rule, CompositionRule):
            raise TypeError("rule must be a CompositionRule")
    ...
    def validate(self, plan: CompositionPlan) -> Dict[str, Any]:
        ...
        for rule in self._rules:
            if rule.check_fn is None:
                continue
            try:
                passed = bool(rule.check_fn(plan))
        ...
        return {
            "valid": not violations and not rule_errors,
