...
class PromptBuilder:
    ...
    def build(
        ...
        if extra_tags is not None and any(
            not isinstance(tag, str) or not tag.strip() for tag in extra_tags
        ):
            raise ValueError("extra_tags must contain non-empty strings")

        style_suffix = self._styles.get(style, style)
        prompt_text = f"{subject.strip()}, {style_suffix}"
        if extra_tags:
            prompt_text += ", " + ", ".join(tag.strip() for tag in extra_tags)
        prompt = ImagePrompt(prompt_text)
        prompt.style = style
        prompt.platform = platform.strip().lower()
    ...
    def from_template(self, template_name: str, **kwargs: Any) -> ImagePrompt:
        ...
        try:
            text = self._templates[template_name].format(**kwargs)
        except (KeyError, ValueError, IndexError) as exc:
            raise ValueError(f"template rendering failed: {exc}") from exc
        return ImagePrompt(text)

    def optimize_for_platform(
        self, prompt: ImagePrompt, platform: str
    ) -> ImagePrompt:
        if not isinstance(prompt, ImagePrompt):
            raise TypeError("prompt must be an ImagePrompt")
        if not isinstance(platform, str) or not platform.strip():
            raise ValueError("platform is required")
        normalized = platform.strip().lower()
        prompt.platform = normalized
        aspect_ratios = {
            "linkedin": "1.91:1",
            "instagram": "1:1",
            "facebook": "1:1",
        ...
        ratio = aspect_ratios.get(normalized)
        if ratio is not None:
            prompt.parameters["aspect_ratio"] = ratio
        return prompt
