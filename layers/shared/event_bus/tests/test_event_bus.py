...
def make_event():
    return Event(event_type=EventType.SYSTEM_STARTUP, data={"ok": True})
...
def test_duplicate_subscription_is_rejected():
    ...
    assert bus.subscribe(EventType.SYSTEM_STARTUP, callback) is True
    assert bus.subscribe(EventType.SYSTEM_STARTUP, callback) is False
    assert bus.get_handler_count(EventType.SYSTEM_STARTUP) == 1


def test_once_and_wildcard_handlers_are_removed_after_dispatch():
    bus = EventBus()
    calls = []
    bus.subscribe(EventType.SYSTEM_STARTUP, lambda event: calls.append("specific"), once=True)
    ...
    assert bus.get_handler_count(EventType.SYSTEM_STARTUP) == 0
    assert bus.get_handler_count() == 1


def test_callback_failure_is_isolated_and_counted():
    bus = EventBus()
    bus.subscribe(EventType.SYSTEM_STARTUP, lambda event: (_ for _ in ()).throw(ValueError("boom")))
    result = bus.publish(make_event())
...
def test_concurrent_publish_and_unsubscribe_are_safe():
    ...
    bus.subscribe(EventType.SYSTEM_STARTUP, callback)
    threads = [
