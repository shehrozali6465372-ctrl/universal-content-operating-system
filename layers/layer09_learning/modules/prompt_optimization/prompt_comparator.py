"""Prompt Comparator — Compare prompt performance between versions."""
...
class PromptComparator:
    """Compare two prompt profiles across multiple metrics."""
    ...
    def compare(self, baseline: PromptProfile, candidate: PromptProfile) -> List[ComparisonResult]:
        self._results.clear()
        metrics = [
            ("success_rate", baseline.success_rate, candidate.success_rate),
            ("avg_engagement", baseline.avg_engagement, candidate.avg_engagement),
            ("avg_quality_score", baseline.avg_quality_score, candidate.avg_quality_score),
            ("avg_confidence", baseline.avg_confidence, candidate.avg_confidence),
    ...
    def get_overall_winner(self, baseline: PromptProfile, candidate: PromptProfile) -> str:
    ...
    def get_significant_differences(self) -> List[ComparisonResult]:
