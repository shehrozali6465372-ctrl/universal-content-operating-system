...
class Prediction:
    ...
    value: float
    trained_on: int
    model_quality_mae: Optional[float]
...
class AutonomousLearningEngine:
    ...
    def __init__(self, db_path: str | Path = "data/layer09_learning.db", *, guardrails: GuardrailConfig | None = None) -> None:
        ...
        self._init_db()
        self.policy_registry = PolicyRegistry(self.db_path)
        self.deployment_guard = DeploymentGuard(self.policy_registry, guardrails)
        ...
        with self._connect() as conn:
            conn.executescript("""
                CREATE TABLE IF NOT EXISTS learning_events (
                    event_id TEXT PRIMARY KEY, scope_key TEXT NOT NULL,
                    platform_id TEXT NOT NULL, niche_id TEXT NOT NULL,
                    audience_id TEXT NOT NULL, country TEXT NOT NULL,
                    language TEXT NOT NULL, content_type TEXT NOT NULL,
                    experiment_id TEXT NOT NULL, model_version TEXT NOT NULL,
                    features_json TEXT NOT NULL, reward REAL NOT NULL, label REAL,
                ...
                CREATE TABLE IF NOT EXISTS model_registry (
                    scope_key TEXT NOT NULL, model_version TEXT NOT NULL,
                    sample_count INTEGER NOT NULL, metric_name TEXT NOT NULL,
                    metric_value REAL NOT NULL, status TEXT NOT NULL,
                    artifact_path TEXT NOT NULL, created_at TEXT NOT NULL,
    ...
    def record_outcome(self, scope: LearningScope, features: Sequence[float], reward: float, *, source: str,
                       experiment_id: str = "", model_version: str = "untrained", label: Optional[float] = None,
                       event_id: Optional[str] = None) -> LearningEvent:
        """Persist one measured outcome. The engine never infers success."""
        if not source.strip():
        ...
        eid = event_id or hashlib.sha256((scope.key + json.dumps(values) + str(reward) + source + experiment_id).encode()).hexdigest()
        event = LearningEvent(eid, scope, values, reward, label, datetime.now(timezone.utc).isoformat(), source)
        with self._connect() as conn:
            conn.execute("""INSERT OR IGNORE INTO learning_events
                (event_id,scope_key,platform_id,niche_id,audience_id,country,language,content_type,
                 experiment_id,model_version,features_json,reward,label,source,created_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                         (eid, scope.key, scope.platform_id, scope.niche_id, scope.audience_id, scope.country,
                          scope.language, scope.content_type, experiment_id, model_version, json.dumps(values),
                          reward, label, source, event.created_at))
        return event
    ...
    def train(self, scope: LearningScope, *, min_samples: int = 20, min_relative_improvement: float = 0.0) -> dict[str, Any]:
        ...
        decision = self.deployment_guard.qualify_candidate(scope, candidate_metric=float(report["metric_value"]),
                                                           baseline_metric=old_mae, evidence_count=len(rows))
        if previous is None and decision.action == "promote":
            status = "active"
        elif previous is not None and decision.action == "promote" and min_relative_improvement <= (decision.relative_improvement or 0.0):
            status = "active"
        else:
            status = "rejected"
        with self._connect() as conn:
            ...
            conn.execute("INSERT OR REPLACE INTO model_registry VALUES (?,?,?,?,?,?,?,?)",
                         (scope.key, version, len(rows), report["metric_name"], report["metric_value"], status,
                          str(artifact), datetime.now(timezone.utc).isoformat()))
        if status == "active":
            self._models[scope.key] = candidate
        return {"status": status, "scope_key": scope.key, "samples": len(rows), "model_version": version,
                "candidate_mae": report["metric_value"], "previous_active_mae": old_mae,
                "validation_samples": report.get("validation_samples"), "decision": decision.action,
    ...
    def predict(self, scope: LearningScope, features: Sequence[float]) -> Prediction:
        ...
        if len(values) != len(getattr(model._model, "n_features_in_", values)):
            raise ValueError("feature dimension does not match active model")
        value = model.predict([values])[0]
        return Prediction(scope.key, model.version, value, model.sample_count, model.mae)
    ...
    def canary_policy(self, scope: LearningScope, policy_id: str, version: int, *, evidence_id: str, evidence_count: int) -> DeploymentDecision:
        return self.deployment_guard.start_canary(scope, policy_id, version, evidence_id=evidence_id, evidence_count=evidence_count)

    def activate_policy(self, scope: LearningScope, policy_id: str, version: int, *, evidence_id: str, evidence_count: int) -> DeploymentDecision:
        return self.deployment_guard.promote(scope, policy_id, version, evidence_id=evidence_id, evidence_count=evidence_count)

    def rollback_policy(self, scope: LearningScope, policy_id: str, version: int, *, reason: str) -> DeploymentDecision:
        return self.deployment_guard.rollback(scope, policy_id, version, reason=reason)

    def active_policy(self, scope: LearningScope) -> PolicyRecord | None:
        return self.policy_registry.get_active(scope)
    ...
    def _rows(self, scope: LearningScope) -> List[sqlite3.Row]:
        with self._connect() as conn:
            return conn.execute("SELECT * FROM learning_events WHERE scope_key=? ORDER BY created_at,event_id", (scope.key,)).fetchall()
